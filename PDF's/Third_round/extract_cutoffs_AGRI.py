            ...
            for idx, line in enumerate(lines):
                ...
                # Now, check for branch + cutoffs
                    ...
                    if cutoff_start is not None:
                        ...
                        # Check for multi-line branch name
                        if (('(' in branch_name and ')' not in branch_name) or len(branch_name) <= 3) and idx+1 < len(lines):
                            ...
                            if not re.match(r'\d+\s+[FN]\d{3}', next_line) and not all(x.isupper() for x in next_parts[:2]):
                                ...
                                skip_lines = 1
                        # Clean and append valid cutoffs
                        for cat, cutoff in zip(categories, cutoffs):
                            if cutoff != '--':
                                try:
                                    cutoff_val = int(cutoff.replace(',', ''))
                                    if 0 < cutoff_val < 2000000:
                                        records.append({
...
# Use like this:
df = pd.DataFrame(parse_agri_pdf("agri_cutoff_2024_hk_ext_rndkannada.pdf"))
df.to_csv("agri_cutoff_2024_hk_ext_rndkannada.csv", index=False)
print("✅ agri_cutoff_2024_hk_ext_rndkannada.csv created successfully.")
